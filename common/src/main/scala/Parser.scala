...
trait Parser extends StandardTokenParsers with PackratParsers with ImplicitConversions {
  ...
  object surface {
    ...
    // [a: term, b: term]
    case class Sigma(seq: Seq[(String, Term)]) extends Term
    // [a = term, b = term]
  ...
  lazy val sigma: PackratParser[surface.Sigma] =
    delimited("{", rep1sep(ident ~ (":" ~> term), ","),"}") ^^ {a => surface.Sigma(a.map(b => (b._1, b._2)))}
  ...
  def transform(a: surface.Module): Module = {
    ...
    def term(a: surface.Term, context: Seq[(String, Int) => Option[Term]]): Term = {
      ...
      def sigmaTerms(seq: Seq[(String, surface.Term)]): Seq[Term] = {
        seq.foldLeft((context, Seq.empty[Term])) { (c, t) =>
          (((n: String, i: Int) => if (t._1 == n) Some(LocalReference(i)) else None) +: c._1, c._2 :+ term(t._2, c._1))
        }._2
      }

      def piToSigmaType(seq: Seq[(String, surface.Term)]): Term = Sigma(seq.indices.map("_" + _), sigmaTerms(seq))

      def piToSigmaContext(seq: Seq[String]) = {
        ((n: String, i: Int) => {
          val index = seq.indexOf(n)
          if (index >= 0) {
            Some(Projection(LocalReference(i), "_" + index))
          } else None
        }) +: context
      }
      ...
      a match {
        case surface.Fix(name, t) =>
          Fix(term(t, ((n: String, i: Int) => if (n == name) Some(LocalReference(i)) else None) +: context))
        ...
        case surface.Pi(seq, body) =>
          ...
          Pi(piToSigmaType(ss), term(body, piToSigmaContext(ss.map(_._1))))
        case surface.Lambda(seq, body) =>
          if (seq.forall(_._2.nonEmpty)) {
            val ts = seq.map(a => (a._1, a._2.get))
            Lambda(Some(piToSigmaType(ts)), term(body, piToSigmaContext(ts.map(_._1))))
          } else if (seq.forall(_._2.isEmpty)) {
            Lambda(None, term(body, piToSigmaContext(seq.map(a => a._1))))
          } else throw new Exception("")
        case surface.App(left, rs) =>
          App(term(left, context), Record(rs.indices.map("_" + _), rs.map(a => term(a, context))))
        case surface.Sigma(seq) =>
          Sigma(seq.map(_._1), sigmaTerms(seq))
        ...
        case surface.Split(t, ts) =>
          Split(term(t, context), ts.map(a => a._1 -> {term(a._3, ((n: String, i: Int) => if(n == a._2) Some(LocalReference(i)) else None) +: context)}).toMap)
