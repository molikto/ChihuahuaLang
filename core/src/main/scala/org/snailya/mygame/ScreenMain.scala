...
trait UntypedArithmeticFrontend extends LanguageFrontendDynamics[UntypedArithmeticAst.Ast, UntypedArithmeticAst.Hole] {
  val UAA = UntypedArithmeticAst
  ...
  override def compile(t: UntypedArithmeticAst.Ast): Either[String, Seq[Error]] = {
    val a = infer(t)
